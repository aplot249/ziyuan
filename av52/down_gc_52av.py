...
def dwz(video_m3u8, oldpath, newpath,video_name):
    ...
    conn = sqlite3.connect("ziyuan.db")
    c = conn.cursor()
    sharelink = c.execute("SELECT sharelink WHERE link = ?",(video_m3u8,))
    dwz = getdwz(sharelink)
    print(dwz)
    c.execute("UPDATE av52_guochan SET dwz = ? WHERE sharelink = ?", (dwz,))
    conn.commit()
    conn.close()
...
if __name__ == '__main__':
    ...
    conn = sqlite3.connect("../ziyuan.db")
    c = conn.cursor()
    db_result = c.execute("SELECT * FROM av52_guochan WHERE token = 0 AND link NOT NULL ")
    result_list = []
    for u in db_result:
